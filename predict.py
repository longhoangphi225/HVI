...
def predict_result(device,cfg,criterion,pb,pf,num_e=None,contexts = []):
    ...
    ckpt_path = "./save_weights/best_weight_"+str(criterion)+"_"+str(mode)+"_"+str(name)+"_"+ str(cfg['TRAIN']['Ray_hidden_dim'])+"_attention.pt"
    ...
    results1 = np.squeeze(np.array(results1, dtype='float32'), axis=-1)
